...
radii_apertures_array = np.array([5., 5.25, 7., 11., 8., 4.72])

locs_apertures_array = np.array([loc_obj, loc_lens, loc_lens, 65.65, 120.25, 137.35])

# path of ray 1: from top of image, optically parallel until lens; then focus at center path (green in drawings)
# path of ray 2: same origin, but going in single path through center of lens to 'top' of image (orange in drawings)
